...
// Syncs AbuseIPDB Top 100 most abused IPs with strict IPv4 validation and bogon filtering
...
const ABUSEIPDB_URL = 'https://raw.githubusercontent.com/borestad/blocklist-abuseipdb/main/abuseipdb-s100-30d.ipv4';
const BATCH_SIZE = 1000; // Small file, can use smaller batches
...
Deno.serve(async (req) => {
  ...
  if (providedSecret && providedSecret !== secret) {
    console.error('[abuseipdb-sync] Invalid cron secret');
  ...
  const networkLogId = await logNetworkCall(supabaseUrl, serviceKey, {
    call_type: 'ingest',
    target_url: ABUSEIPDB_URL,
    target_name: 'AbuseIPDB Top 100',
    method: 'GET',
    edge_function_name: 'abuseipdb-top100-sync',
  });

  try {
    console.log('[abuseipdb-sync] 🚀 Starting AbuseIPDB Top 100 sync...');
    console.log(`[abuseipdb-sync] 📥 Fetching from: ${ABUSEIPDB_URL}`);

    // === PHASE 1: Download file ===
    const response = await fetch(ABUSEIPDB_URL, {
      headers: {
        'User-Agent': 'IntelGuard/AbuseIPDB-Sync/1.0',
    ...
    console.log(`[abuseipdb-sync] ✅ Downloaded ${(fileContent.length / 1024).toFixed(1)}KB`);
    ...
    console.log(`[abuseipdb-sync] 📊 Parsed ${totalLines} lines:`);
    console.log(`[abuseipdb-sync]   ✓ Valid public IPs: ${validIPs}`);
    console.log(`[abuseipdb-sync]   🚫 Filtered bogons/private: ${filteredBogons}`);
    console.log(`[abuseipdb-sync]   ❌ Invalid format: ${filteredInvalid}`);
    ...
    if (validIPsBatch.length > 0) {
      ...
      for (let i = 0; i < validIPsBatch.length; i += BATCH_SIZE) {
        ...
        await upsertIPBatch(supabase, batch, 'abuseipdb_top100');
      }
    }
    ...
    console.log(`[abuseipdb-sync] ✅ COMPLETED in ${totalTime}ms`);
    console.log(`[abuseipdb-sync] 📈 Accept rate: ${acceptRate}%`);
    ...
    console.error('[abuseipdb-sync] ❌ ERROR:', errorMsg);
